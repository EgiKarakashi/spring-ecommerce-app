...
const ProductAttributes: NextPage = () => {
  ...
  const addNewAttributeOfProduct = (event: React.MouseEvent<HTMLElement>) => {
    ...
    setListDeleteProductAttributeId(
      listDeleteProductAttributeId.filter((item) => item.valueOf().toString() !== productAttributeId.toString())
    )
  ...
  const saveProductAttributeOfProduct = async (event: any) => {
    ...
    if (id) {
      router.push("/catalog/products").catch((error) => console.log(error))
    }
  }
        ...
        <div>
          <div className="mb-3">
            ...
            <select
              ...
              id="attribute-name"
              onChange={(event) => {
                setAttributeName(event.target.value.split("/")[0]);
                setEditProductAttributeId(event.target.value.split("/")[1]);
              }}
            >
            ...
            {attributeOfProducts?.map((productValue) => (
              <React.Fragment key={productValue.id}>
                ...
                >
                  ...
                  <td>
                    <button className="btn btn-outline-danger" type="submit" onClick={deleteAttributeOfProduct}>
                      Delete
                    </button>
                  </td>
                </tr>
